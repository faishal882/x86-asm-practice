...
;                   (Practice Program Jeff Dntemann)
;
; Build using these commands:
;   nasm -f elf -g -F stabs eatterm.asm
;   ld -m elf_i386 -s -o eatterm eatterm.o
;
;
...
_start:
  ...
  mov al,12           ; Specy Line
  mov ecx,AdMsg       ; Pass address of message
  mov edx,ADLEN       ; Pass length of msg
  call WriteCtr       ; Display it to console

  ; Position the cursor for "Press Enter" prompt:
  mov ax,0117h        ; X,Y = 1,23 as a single hex vale in hex
  call GotoXY         ; Position the cursor

  ; Display the "press Enter" prompt:
  mov ecx,Prompt      ; Pass offset of prompt
  mov edx,PROMPTLEN   ; Pass length of prompt
  call WriteStr       ; Send prompt to console

  ; Wait for the user to press Enter:
  mov eax,3           ; Code for sys_read
  mov ebx,0           ; Specify FIle Desc. 0: Stdin
  int 80h             ; Make Kernel call

  ; Done
  Exit:
    mov eax,1         ; Code for Exit syscall
    mov ebx,0         ; Return code to zero
    int 80h           ; Make kernel call
